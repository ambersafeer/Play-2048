...
public class Board {
    ...
    public void addRandomTile() {
        ...
        int col = openSpaces.get(randomOpenSpace).getCol();
        int row = openSpaces.get(randomOpenSpace).getRow();
        gameBoard[row][col] = (int) value;
    ...
    public void swipeLeft() {
        ...
        for (int i = 0; i < gameBoard.length; i++){
            for (int j = 0; j < gameBoard[i].length; j++){
                if (gameBoard[i][j] == 0){
                    for (int counter = j+1; counter < gameBoard[i].length; counter++){
                        if (gameBoard[i][counter] != 0){
                            int temp = gameBoard[i][j];
                            gameBoard[i][j] = gameBoard[i][counter];
                            gameBoard[i][counter] = temp;
                            break;
    ...
    public void flipRows() {
        ...
        for (int i = 0; i < gameBoard.length; i++){
            for (int j = 0; j < gameBoard[i].length/2; j++){
                int temp = gameBoard[i][j];
                gameBoard[i][j] = gameBoard[i][gameBoard.length - j - 1];
                gameBoard[i][gameBoard.length - j -1] = temp;
            }
        }
    }
